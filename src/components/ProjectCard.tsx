...
const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(({ project, index }, ref) => {
  ...
  return (
    ...
    >
      ...
      <p className="hidden lg:block max-w-xs text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        {project.description}
      </p>

      {/* Hover Image Preview */}
      <div className="absolute right-12 top-1/2 -translate-y-1/2 w-32 h-20 md:w-48 md:h-28 rounded-xl overflow-hidden opacity-0 scale-50 group-hover:opacity-100 group-hover:scale-100 transition-all duration-500 pointer-events-none border-2 border-swiss-black shadow-[4px_4px_0px_0px_rgba(26,26,26,1)]">
        <img
